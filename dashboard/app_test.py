# Clicks on website per 5 minutes. Add data display. update deque size.
...
def reactive_calc_combined():
    ...
    clicks = round(random.uniform(1, 25), 1)
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    new_dictionary_entry = {"clicks":clicks, "timestamp":timestamp}
...
with ui.card():
    ui.card_header("Line Chart all data")

    @render_plotly
    def display_line():
        # define data values
        x = df['timestamp']  # X-axis points
        y = df['temp'] # Y-axis points
         
        fig = px.scatter(x=x, y=y, title="Line Chart all data", labels={"x": "X-axis", "y": "Y-axis"})
        return fig